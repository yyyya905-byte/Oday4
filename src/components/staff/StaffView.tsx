          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 max-w-xl leading-relaxed">
              {isGoogleSignedIn && googleUser ? (
                <span>
                  مسجل الدخول حالياً بحساب: <strong className="font-mono text-slate-900 dark:text-white">{googleUser.email}</strong> ({googleUser.name}). يتيح لك هذا الحساب التحكم الكامل ومزامنة البيانات مع Google Drive.
                </span>
              ) : (
                <span>
        ...
        <div className="flex items-center gap-2 self-start md:self-auto shrink-0 flex-wrap">
          {isGoogleSignedIn && googleUser ? (
            ...
            <>
              ...
              <button
                onClick={() => signInWithGoogle({ hintEmail: USER_METADATA_EMAIL })}
                disabled={isGoogleAuthLoading}
                ...
                title={`دخول فوري بحساب: ${USER_METADATA_EMAIL}`}
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                <span>حساب المالك ({USER_METADATA_EMAIL.split('@')[0]})</span>
              </button>
            </>
          )}
        </div>
