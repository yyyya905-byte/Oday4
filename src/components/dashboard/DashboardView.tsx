...
export const DashboardView: React.FC = () => {
  ...
  const last7DaysData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayName = d.toLocaleDateString(language === 'ar' ? 'ar-SY' : 'en-US', { weekday: 'short' });
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                حركة المبيعات خلال الأيام الـ 7 الماضية
              </h3>
              <p className="text-xs text-slate-400">مجموع المبيعات اليومية المحققة</p>
            </div>
          </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={last7DaysData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(val: any) => [formatCurrency(Number(val)), 'المبيعات']}
