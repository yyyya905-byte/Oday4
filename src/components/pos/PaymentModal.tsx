            ...
            <div className="p-5 sm:p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              {paymentMethod !== 'credit' && (
                <div className="space-y-3 bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {t('exactAmount')} ({formatCurrency(totalAmount)})
                    </button>
                  </div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={paidAmount || ''}
                      onChange={e => setPaidAmount(Number(e.target.value))}
                      className="w-full text-2xl font-black font-mono py-3 px-4 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl border-2 border-amber-500/50 focus:border-amber-500 focus:outline-none text-start shadow-inner"
                      autoFocus
                    />
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
                      {quickBanknotes.map(val => (
                        <button
                          ...
                          className="py-2 px-1.5 bg-white dark:bg-slate-900 hover:bg-amber-50 dark:hover:bg-amber-950/40 text-xs font-bold font-mono text-slate-800 dark:text-slate-200 rounded-xl border border-slate-200 dark:border-slate-700 transition-all active:scale-95 text-center shadow-2xs cursor-pointer"
                        >
                          {val.toLocaleString()}
                        </button>
                      ))}
                  ...
                  <div className="flex items-center gap-1.5 pt-1">
                    ...
                    {[5000, 10000, 25000, 50000].map(inc => (
                      <button
                        ...
                        className="px-2 py-1 bg-slate-200/80 dark:bg-slate-700/80 hover:bg-amber-100 dark:hover:bg-amber-900/40 text-[11px] font-mono font-bold text-slate-700 dark:text-slate-300 rounded-lg transition-colors cursor-pointer"
                      >
                        +{inc.toLocaleString()}
                      </button>
                    ))}
              ...
              <div>
                <input
                  ...
                  className="w-full text-xs py-2.5 px-3.5 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-amber-500"
                />
              </div>
              ...
              <button
                ...
                className={`w-full py-4 rounded-2xl text-white font-extrabold text-sm flex items-center justify-center gap-2 shadow-lg transition-all active:scale-98 cursor-pointer ${
                  (paymentMethod === 'cash' && paidAmount < totalAmount) ||
                  (paymentMethod === 'credit' && !selectedCustomer)
