...
import { UtensilsCrossed, Users, Sparkles, Clock, MapPin, Coffee, Check, ChevronDown, Plus } from 'lucide-react';
...
export const RestaurantPOSHeader: React.FC<RestaurantPOSHeaderProps> = ({ onOpenKitchenTicket }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-800 to-teal-900 text-white rounded-3xl p-3 sm:p-4 shadow-md border border-emerald-700/50 space-y-3">
      {/* Top Row: Dining Mode Selector & Captain Info */}
      <div className="flex flex-wrap items-center justify-between gap-2.5">
        {/* Dining Type Buttons */}
        <div className="flex items-center bg-emerald-950/70 p-1 rounded-2xl border border-emerald-700/60 shadow-inner">
          <button
            type="button"
        ...
        <div className="flex items-center gap-2 text-xs font-semibold">
          <div className="flex items-center gap-1.5 bg-emerald-950/60 px-3 py-1.5 rounded-xl border border-emerald-700/40 text-emerald-100">
          ...
          <div className="hidden md:flex items-center gap-1 bg-emerald-950/60 px-2.5 py-1.5 rounded-xl border border-emerald-700/40 text-emerald-200 font-mono text-[11px]">
      ...
      {restaurantDiningType === 'dine_in' ? (
        <div className="relative flex flex-wrap items-center justify-between gap-2.5 pt-2 border-t border-emerald-700/40">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-emerald-700/90 hover:bg-emerald-600 px-3.5 py-1.5 rounded-xl text-xs font-bold text-white border border-emerald-500/60 shadow-xs transition-all active:scale-95"
              >
            ...
            <div className="flex items-center gap-1.5 bg-emerald-950/70 px-2.5 py-1 rounded-xl border border-emerald-700/50 text-xs">
              <Users className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-[11px] text-emerald-200">{language === 'ar' ? 'الضيوف:' : 'Guests:'}</span>
              <button
                type="button"
                onClick={() => setGuestCount(Math.max(1, guestCount - 1))}
                className="w-5 h-5 rounded-lg bg-emerald-800 hover:bg-emerald-700 text-white font-bold flex items-center justify-center"
              >
                -
              </button>
              <span className="font-mono font-bold text-white px-1.5">{guestCount}</span>
              <button
                type="button"
                onClick={() => setGuestCount(guestCount + 1)}
                className="w-5 h-5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white font-bold flex items-center justify-center"
              >
                +
              </button>
            </div>
          ...
          <div className="hidden sm:flex items-center gap-1 text-xs">
            <span className="text-[10px] text-emerald-300">{language === 'ar' ? 'طاولات سريعة:' : 'Quick:'}</span>
        ...
        <div className="flex items-center justify-between text-xs text-emerald-200 pt-2 border-t border-emerald-700/40">
          <div className="flex items-center gap-2">
            <span className="bg-emerald-950/80 px-2.5 py-1 rounded-xl text-emerald-300 font-bold border border-emerald-700/40">
