...
import { Shield, KeyRound, X, Delete, ShieldCheck, Zap, UserCheck, LogOut, CheckCircle2, RefreshCw, Sparkles, LogIn } from 'lucide-react';
import { getRoleInfo } from '../../utils/permissions';
import { GoogleIcon } from '../common/GoogleIcon';
import { USER_METADATA_EMAIL } from '../../services/googleAuthService';
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-black text-sm text-slate-900 dark:text-white">
                تسجيل الدخول وتبديل المستخدم
              </h3>
              <p className="text-[11px] text-slate-400">
                الدخول عبر حساب Google أو رمز المرور الشخصي (PIN)
              </p>
            </div>
          </div>
          <button
          ...
          <div className="bg-gradient-to-br from-slate-50 to-blue-50/40 dark:from-slate-800/50 dark:to-slate-800/20 p-3.5 rounded-2xl border border-blue-100/80 dark:border-slate-700/60 shadow-xs text-start">
            {isGoogleSignedIn && googleUser ? (
              ...
              <div className="space-y-2.5">
                ...
                {/* Quick 1-click button for the verified user */}
                <button
                  type="button"
                  onClick={() => handleGoogleSignIn(USER_METADATA_EMAIL)}
                  disabled={isGoogleAuthLoading}
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>دخول فوري بحساب:</span>
                    <span className="font-mono font-bold text-slate-900 dark:text-white">{USER_METADATA_EMAIL}</span>
                  </span>
                  <span className="text-[10px] font-black text-blue-600 dark:text-blue-400">انقر هنا ⚡</span>
                </button>
              </div>
            )}
          </div>
